...
// Simple hospital system. Students: this is the app 'brain'.
...
public class HospitalSystem {
    ...
    // small helper: send message to registered GUI logger (if present)
    ...
    // Determine whether a given ongoing patient should be confined to a room.
    // This is a simple, explainable scoring function that combines severity, vitals,
    // age/comorbidity, trend, and resource availability.
    public ConfinementDecision shouldConfine(Patient p, String deptName) {
        ...
        // Trend (positive=waning health)
        if (p.getTrendScore() != null && p.getTrendScore() > 0.0) {
            score += p.getTrendScore() * 2.0; // tunable multiplier
            reasons.append("worsening;");
        }

        // Time in ongoing (if we can infer from Department ongoing entries, caller may set a started timestamp)
        // We'll not depend on started time here because Department holds start; callers can add extra score if needed.

        // Resource gating: is there at least one free room? If none, decline with reason.
    ...
    // compute an estimated release date based on the patient's arrival time + confinement duration
        // compute using Philippine time so GUI PH Time and release estimations align
    ...
    // store the estimated release time now so released records / logs can show the future date immediately
    ...
    public boolean releaseConfinedPatient(String deptName, int patientId) {
        ...
        // Set release time to reflect how long the patient stayed (arrival + confinementDays)
