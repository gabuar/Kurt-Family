...
public class HospitalGUI extends JFrame implements LogListener {
    ...
    // Include year for release timestamps so future release dates are obvious
    private final DateTimeFormatter releaseDtf = DateTimeFormatter.ofPattern("MM-dd-yyyy HH:mm");

    // Small helper to create a cell renderer that shows full cell text as tooltip
    ...
    private void startSimulation() {
        ...
        simTimer.start();
    // simulation started: refresh stats display
    refreshStats();
    }
    ...
    private void stopSimulation() {
        ...
        simTimer.stop();
    // simulation stopped: refresh stats display
    refreshStats();
    }

    // Called every simulation tick (driven by simTimer)
    private void simTick() {
        // 1) Randomly generate new ER arrivals with a small probability
        if (simRand.nextDouble() < 0.4) {
            Patient p = createRandomPatient();
            system.addPatientToER(p);
        }

        // 2) Move some ER patients into departments at random (if any exist)
        ...
        // 3) Auto-fill departments (assign doctors to waiting patients)
        system.autoFillAllDepartments();

        // 4) Randomly release some ongoing patients (simulate completed treatments)
        for (String deptName : system.getDepartments().keySet()) {
            ...
            for (Department.OngoingEntry entry : ongoing) {
                // higher severity takes longer; so lower chance to release; CRITICAL least likely
                double base = 0.25;
                ...
                if (simRand.nextDouble() < base) {
                    ...
                    // small chance to confine an ongoing patient into a room
        ...
        // 5) Occasionally release confined patients whose confinementDays have expired (approx)
        for (String deptName : system.getDepartments().keySet()) {
            Department dept = system.getDepartments().get(deptName);
            List<Patient> confined = dept.getConfinedSnapshot();
            for (Patient p : confined) {
                // if the patient's stored releaseTime is in the past, release them
    ...
    // --- UI Colors (for a more seamless design) ---
    ...
    // Map from doctor id to a generated display name to avoid repeated identical names
    ...
    // Generate a reasonably unique doctor name using first+last combinations and track used names
    ...
    // Department descriptions shown in the UI
    private final Map<String, String> deptDescriptions = new LinkedHashMap<>();

    // --- Time Indicator (Philippines Time) ---
    ...
    private Timer simTimer; // Swing timer for simulation ticks
    private int simIntervalMs = 1000; // base interval (ms) between simulation ticks
   ...
   private static final String[] PATIENT_NAMES = {
    ...
    public HospitalGUI() {
        ...
        getContentPane().setBackground(BACKGROUND_COLOR); // Use defined background color

    JTabbedPane tabs = new JTabbedPane();
    tabs.setBackground(SECONDARY_COLOR); // Use defined secondary color
    tabs.setForeground(TEXT_COLOR); // Set tab text color
    // make tabs less tall by using a smaller font and compact tab insets
    tabs.setFont(tabs.getFont().deriveFont(Font.PLAIN, 11f));
    tabs.setTabLayoutPolicy(JTabbedPane.SCROLL_TAB_LAYOUT);
    UIManager.put("TabbedPane.tabInsets", new Insets(4,8,4,8));

    // Simulation control panel (top-right)
    ...
    // Add the tabs and also show controls above via a wrapper panel
    ...
    add(centerWrapper, BorderLayout.CENTER);
        ...
        // --- Status Bar with Time Indicator ---
        JPanel statusBar = new JPanel(new FlowLayout(FlowLayout.RIGHT, 10, 5)); // Added padding
        statusBar.setBackground(SECONDARY_COLOR);
        statusBar.setBorder(new EmptyBorder(5, 10, 5, 10)); // Add some padding
        ...
        startPhilippineTimeUpdater(); // Start the time updater

        // Simulation timer (ticks drive the autonomous simulation)
    ...
    // no verbose logs panel anymore; we keep records/reports concise
        refreshAll();
        setVisible(true);
    }
    ...
    private void startPhilippineTimeUpdater() {
        Timer timer = new Timer(1000, e -> updatePhilippineTime()); // Update every 1 second
    ...
    private JPanel createERPanel() {
        ...
        JPanel top = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 5)); // Added padding
        top.setOpaque(false);
        ...
        erTable.getTableHeader().setBackground(SECONDARY_COLOR); // Style table header
        erTable.getTableHeader().setForeground(PRIMARY_COLOR);
        erTable.setFillsViewportHeight(true); // Make table fill scrollpane

        JScrollPane erScroll = new JScrollPane(erTable);
        erScroll.setWheelScrollingEnabled(true);
        erScroll.setBorder(new LineBorder(BORDER_COLOR, 1)); // Consistent border
        ...
        assignSev.addActionListener(e -> {
            ...
            if (newSev != null) {
                ...
                // Re-add to triage queue to re-sort based on new severity
                system.getTriageQueue().getQueue().remove(patient); // Remove old entry
                system.getTriageQueue().addPatient(patient); // Add new entry (will re-sort)
                refreshAll();
            }
        });
    ...
    private JScrollPane createDepartmentsPanel() {
        ...
        outer.setBorder(new EmptyBorder(10,10,10,10)); // Add padding to the outer panel

        for (String deptName : system.getDepartments().keySet()) {
            Department dept = system.getDepartments().get(deptName);

            JPanel card = new JPanel(new BorderLayout(6,6));
            card.setBackground(Color.white); // Department cards are white
            card.setBorder(new LineBorder(BORDER_COLOR, 1, true)); // Rounded border

            // use a titled border so department name is always visible and styled
            TitledBorder tb = BorderFactory.createTitledBorder(
                new LineBorder(BORDER_COLOR, 1, true), // Use rounded line border
                deptName,
                TitledBorder.LEFT,
                TitledBorder.TOP,
                new Font("SansSerif", Font.BOLD, 16),
                PRIMARY_COLOR // Use primary color for title
            );
            tb.setTitleJustification(TitledBorder.CENTER); // Center the title
            card.setBorder(BorderFactory.createCompoundBorder(tb, new EmptyBorder(10,10,10,10))); // Add padding inside border

            // small title label with tooltip + description under the title
            String desc = deptDescriptions.getOrDefault(deptName, "General department services and specialist consultations.");
            // Removed the separate titleLabel and descLabel as TitledBorder handles the main title
            // and the description can be a tooltip for the card itself or a smaller label if needed.
            // For now, let's keep the description as a tooltip for the card.
            ...
            subHeaders.setBorder(new EmptyBorder(0,0,5,0)); // Add some bottom padding
            ...
            card.add(subHeaders, BorderLayout.NORTH); // Changed to NORTH to be below the TitledBorder
            ...
            right.setBorder(new EmptyBorder(0, 10, 0, 0)); // Padding on the left for separation
            ...
            docList.setCellRenderer(new DoctorListCellRenderer()); // Custom renderer for doctors
            ...
            right.add(Box.createRigidArea(new Dimension(0,10))); // More spacing
            ...
            styleButton(autoFillBtn);
            styleButton(releaseBtn);
            styleButton(confineBtn);
            styleButton(toggleDocBtn);
            styleButton(addDocBtn);
            ...
            confineBtn.addActionListener(e -> {
                ...
                roomBox.setRenderer(new DefaultListCellRenderer() {
                    ...
                    public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                        ...
                        else c.setForeground(ACCENT_COLOR); // Use accent color for available rooms
                        return c;
                    }
                });
        ...
        sc.setBorder(BorderFactory.createEmptyBorder()); // Remove scroll pane border
        return sc;
    }
    ...
    private JPanel createConfinedPanel() {
        ...
        // Allow horizontal scrolling so long ConfinementInfo text can be viewed
        confinedTable.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
        // set reasonable preferred widths per column (ID,Name,Age,Case,Severity,Dept,Arrival,Room,Days,ConfinementInfo)
        ...
        // renderer to show full text on hover for ConfinementInfo
    ...
    confScroll.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
        ...
        extendBtn.addActionListener(e -> {
            ...
            // Build extend dialog with days, months, years
            ...
            // Apply extension to the Patient object
            target.setConfinementDays(target.getConfinementDays() + extraDays);
            // update confinementInfo with new est release
    ...
    private JPanel createRecordsPanel() {
        ...
        recordsSearchField.setBorder(new LineBorder(BORDER_COLOR, 1)); // Style text field
        ...
        // allow horizontal scrolling so ReleasedAt and ConfinementInfo columns are readable
        recTable.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
    if (recTable.getColumnModel().getColumnCount() >= 8) {
            recTable.getColumnModel().getColumn(0).setPreferredWidth(50);   // ID
            recTable.getColumnModel().getColumn(1).setPreferredWidth(160);  // Name
            recTable.getColumnModel().getColumn(2).setPreferredWidth(50);   // Age
            recTable.getColumnModel().getColumn(3).setPreferredWidth(140);  // Case
            recTable.getColumnModel().getColumn(4).setPreferredWidth(140);  // Dept
            recTable.getColumnModel().getColumn(5).setPreferredWidth(90);   // Status
            recTable.getColumnModel().getColumn(6).setPreferredWidth(120);  // ReleasedAt
            recTable.getColumnModel().getColumn(7).setPreferredWidth(360);  // ConfinementInfo
        }
        ...
        // set tooltip renderer for ReleasedAt and ConfinementInfo
        if (recTable.getColumnModel().getColumnCount() >= 8) {
    ...
    // Logs have been removed in favor of concise Records/Reports. No logs panel.
    ...
    private void styleButton(JButton button) {
        ...
        button.setBorder(BorderFactory.createCompoundBorder(
            new LineBorder(ACCENT_COLOR.darker(), 1, true), // Slightly darker border, rounded
            new EmptyBorder(5, 15, 5, 15) // Padding
        ));
    ...
    private static class SeverityCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table,Object value,boolean isSelected,boolean hasFocus,int row,int column){
            ...
            if(value!=null){
                ...
                if(sev.equalsIgnoreCase("VERY_MINOR") || sev.equalsIgnoreCase("MILD")) bg = new Color(198,239,206); // Light Green
                else if(sev.equalsIgnoreCase("MODERATE")) bg = new Color(255,235,156); // Light Yellow
                else if(sev.equalsIgnoreCase("SEVERE")) bg = new Color(255,199,206); // Light Red
                else if(sev.equalsIgnoreCase("CRITICAL")) bg = new Color(255,120,120); // Red
    ...
    // Custom ListCellRenderer for Doctor list to show availability
        ...
        public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            ...
            if (value instanceof String) {
                ...
                if (text.contains("Open Slot")) {
                    c.setForeground(ACCENT_COLOR.darker()); // Green for available
                } else if (text.contains("Treating:")) {
                    // Mark doctors who are currently treating as red
                    c.setForeground(Color.RED.darker());
                } else if (text.contains("Busy")) {
                    c.setForeground(Color.RED.darker()); // Red for busy
                } else {
    ...
    private void refreshAll(){
        ...
        for(String deptName: system.getDepartments().keySet()){
            ...
            for(Doctor doc: pool){
                String extra = "";
                // Check if this doctor is currently treating a patient
                for(Department.OngoingEntry entry: ongoing){
        ...
        // Records: include arrival time and released time in the model
    ...
    private void refreshStats(){
        ...
        for(Map.Entry<String,Integer> e: stats.entrySet()){
            ...
            row.setBorder(BorderFactory.createMatteBorder(0,0,1,0,BORDER_COLOR)); // Consistent border
            statsPanel.add(row);
        }
